...
class User extends Db
{
     ...
     */
    public function createUser(string $name, string $kana, string $email, string $password)
    {
        try{
    ...
    /**
     * ログイン可能なユーザーか判定を行ってユーザーIDを返却する
     *
     * @param string $email 登録するメールアドレス
     * @param string $password 登録するパスワード
     * @return array|false ['id' => 'ユーザーID'] または ログイン不能な場合はfalseを返却
     */
     ...
     */
    public function updateUser(string $id, string $name, string $kana, string $email, string $password = null): bool
    {
        try{
            // 重複アドレスの確認 (メールアドレスが一意のためすでに使用されていた場合はエラーとする)
